...
package app.umitems.healthclock.timer;
...
import app.umitems.healthclock.CircleButtonsLayout;
import app.umitems.healthclock.DeskClock;
import app.umitems.healthclock.DeskClockFragment;
import app.umitems.healthclock.LabelDialogFragment;
import app.umitems.healthclock.R;
import app.umitems.healthclock.TimerSetupView;
import app.umitems.healthclock.Utils;
import app.umitems.healthclock.widget.sgv.GridAdapter;
import app.umitems.healthclock.widget.sgv.SgvAnimationHelper;
import app.umitems.healthclock.widget.sgv.StaggeredGridView;
...
public static final  String GOTO_SETUP_VIEW    = "healthclock.timers.gotosetup";
...
class ClickAction {
	...
	public
	ClickAction(int action, TimerObj t) {
		mAction = action;
		mTimer = t;
	}
}
...
class TimersListAdapter extends GridAdapter {
	...
	public
	TimersListAdapter(Context context, SharedPreferences prefs) {
		mContext = context;
		mmPrefs = prefs;
	}
		...
		for (int i = 0; i < mTimers.size(); i++) {
			...
			if(t.mTimerId == id) {
				...
				if(mTimers.size() == 1 && mColumnCount > 1) {
					...
					mTimersList.setAnimationMode(SgvAnimationHelper.AnimationIn.FADE,
					                             SgvAnimationHelper.AnimationOut.FADE);
				}
				else {
					mTimersList.setAnimationMode(SgvAnimationHelper.AnimationIn.FLY_IN_NEW_VIEWS,
					                             SgvAnimationHelper.AnimationOut.FADE);
				}
	...
	protected
	int findTimerPositionById(int id) {
		for (int i = 0; i < mTimers.size(); i++) {
			TimerObj t = mTimers.get(i);
			if(t.mTimerId == id) {
				return i;
	...
	public
	void removeTimer(TimerObj timerObj) {
...
View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
	...
	FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) mAddTimer.getLayoutParams();
...
void onResume() {
	...
	if(getActivity() instanceof DeskClock) {
		...
		View footerView = getActivity().getLayoutInflater().inflate(R.layout.blank_footer_view,
		                                                            mTimersList,
		                                                            false);
...
private
void onClickHelper(ClickAction clickAction) {
...
private
void onPlusOneButtonPressed(TimerObj t) {
	switch (t.mState) {
		case TimerObj.STATE_RUNNING:
...
private
void onStopButtonPressed(TimerObj t) {
	switch (t.mState) {
		case TimerObj.STATE_RUNNING:
