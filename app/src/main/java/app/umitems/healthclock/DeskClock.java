...
import app.umitems.healthclock.stopwatch.StopwatchService;
import app.umitems.healthclock.stopwatch.Stopwatches;
import app.umitems.healthclock.timer.TimerFragment;
import app.umitems.healthclock.timer.TimerObj;
import app.umitems.healthclock.timer.Timers;
import app.umitems.healthclock.worldclock.CitiesActivity;

/**
 * DeskClock clock view for desk docks.
 */
public
class DeskClock extends Activity
		implements LabelDialogFragment.TimerLabelDialogHandler, LabelDialogFragment.AlarmLabelDialogHandler {
...
public static final String SELECT_TAB_INTENT_EXTRA = "healthclock.select.tab";

public static String labelCodePHA    = "";
public static String labelFromIntent = "";
public static int       HrFromIntent;
public static int       MinFromIntent;
public static int       PHA_CODE;
//public static Context sContext = this;
...
public static final int RTL_ALARM_TAB_INDEX     = 2;
...
private
void createTabs(int selectedIndex) {
...
void onCreate(Bundle icicle) {
	...
	Log.d("XXX2", "X2");
	try {
		labelFromIntent = getIntent().getStringExtra("PHA-label");

		PHA_CODE = getIntent().getIntExtra("PHA-Code",-1);
		HrFromIntent = getIntent().getIntExtra("PHA-Hr", 99);
		MinFromIntent = getIntent().getIntExtra("PHA-Min", 0);
		Log.d("[XXXX]",String.valueOf(PHA_CODE)+";"+String.valueOf(HrFromIntent));
	}
	catch (Exception e) {
...
public
void clockButtonsOnClick(View v) {
	if(v == null) {
		return;
	}
	switch (v.getId()) {
...
private
boolean processMenuClick(MenuItem item) {
...
public
void registerPageChangedListener(DeskClockFragment frag) {
	if(mTabsAdapter != null) {
...
public
void unregisterPageChangedListener(DeskClockFragment frag) {
	if(mTabsAdapter != null) {
...
class TabsAdapter extends FragmentPagerAdapter
	...
	Fragment getItem(int position) {
		...
		DeskClockFragment f = (DeskClockFragment) Fragment.instantiate(mContext,
		                                                               info.clss.getName(),
		                                                               info.args);
		return f;
	}
	...
	public
	void addTab(ActionBar.Tab tab, Class<?> clss, int position) {
	...
	@Override
	public
	void onTabReselected(Tab arg0, FragmentTransaction arg1) {
		// Do nothing
	}
	...
	@Override
	public
	void onTabUnselected(Tab arg0, FragmentTransaction arg1) {
		// Do nothing
	}

	public
	void notifySelectedPage(int page) {
		notifyPageChanged(page);
	}

	private
	void notifyPageChanged(int newPage) {
	...
	@TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1) private
	boolean isRtl() {
		return TextUtils.getLayoutDirectionFromLocale(Locale.getDefault()) == View.LAYOUT_DIRECTION_RTL;
	}

	private
	int getRtlPosition(int position) {
		if(isRtl()) { //isRtl()
			switch (position) {
...
class OnTapListener implements OnTouchListener {
	...
	public
	OnTapListener(Activity activity, TextView makePressedView) {
