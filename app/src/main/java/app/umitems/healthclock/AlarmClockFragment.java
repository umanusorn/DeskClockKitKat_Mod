...
import com.umitems.datetimepicker.time.RadialPickerLayout;
import com.umitems.datetimepicker.time.TimePickerDialog;
...
import app.umitems.healthclock.alarms.AlarmStateManager;
import app.umitems.healthclock.provider.Alarm;
import app.umitems.healthclock.provider.AlarmInstance;
import app.umitems.healthclock.provider.DaysOfWeek;
import app.umitems.healthclock.widget.ActionableToastBar;
import app.umitems.healthclock.widget.TextTime;

/**
 * AlarmClock application.
 */
public
class AlarmClockFragment extends DeskClockFragment
		implements LoaderManager.LoaderCallbacks<Cursor>, TimePickerDialog.OnTimeSetListener, View.OnTouchListener {
...
public static final String ALARM_CREATE_NEW_INTENT_EXTRA = "healthclock.create.new";
...
public static final String SCROLL_TO_ALARM_INTENT_EXTRA = "healthclock.scroll.to.alarm";
...
void onCreate(Bundle savedState) {
	...
	String s;
	//s= (String)icicle.get("StringTest");
	android.util.Log.d("XXX", "X3");
	try {
		Log.d("XXXXXX");
		//Log.d(String.valueOf(DeskClock.PHA_CODE));

		if(DeskClock.HrFromIntent<=24) {
		//	Log.d(DeskClock.labelCodePHA);
			createNewAlarm(DeskClock.labelFromIntent, DeskClock.HrFromIntent, DeskClock.MinFromIntent);
		}
	}
	catch (Exception e) {
		Log.e(e.toString());
...
View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedState) {
	...
	if(mTimelineLayout != null) {
		...
		mTimelineViewWidth =
				getActivity().getResources().getDimensionPixelOffset(R.dimen.alarm_timeline_layout_width);
	}
			...
			if(count == 0) {
				...
				if(mTimelineLayout != null && mAlarmsView != null) {
					...
					if(prevAdapterCount > 0) {
						...
						ValueAnimator animator = ValueAnimator.ofFloat(1f, 0f).setDuration(ANIMATION_DURATION);
						...
						animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
							...
							void onAnimationUpdate(ValueAnimator animator) {
								...
								setUndoBarRightMargin(currentTimelineWidth + mUndoBarInitialMargin);
							}
						});
					...
					if(prevAdapterCount == 0) {
						...
						animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
							...
							void onAnimationUpdate(ValueAnimator animator) {
								...
								mTimelineLayout.setAlpha(value);
								mTimelineLayout.requestLayout();
								((FrameLayout.LayoutParams) mAlarmsView.getLayoutParams()).setMargins(0,
								                                                                      0,
								                                                                      (int) -rightOffset,
								                                                                      0);
...
private
void setUndoBarRightMargin(int margin) {
	FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) mUndoBar.getLayoutParams();
	((FrameLayout.LayoutParams) mUndoBar.getLayoutParams()).setMargins(params.leftMargin,
	                                                                   params.topMargin,
	                                                                   margin,
	                                                                   params.bottomMargin);
...
private
void hideUndoBar(boolean animate, MotionEvent event) {
	if(mUndoBar != null) {
...
public
void createNewAlarm(String label, int hourOfDay, int minute) {
...
private
void showLabelDialog(final Alarm alarm) {
	...
	final LabelDialogFragment newFragment = LabelDialogFragment.newInstance(alarm,
	                                                                        alarm.label,
	                                                                        getTag());
...
public
void setLabel(Alarm alarm, String label) {
	alarm.label = label;
...
@Override
public
void onLoadFinished(Loader<Cursor> cursorLoader, final Cursor data) {
 ...
 */
private
void scrollToAlarm(long alarmId) {
	int alarmPosition = -1;
	...
	if(alarmPosition >= 0) {
		...
		Toast toast = Toast.makeText(context,
		                             R.string.missed_alarm_has_been_deleted,
		                             Toast.LENGTH_LONG);
...
private
void launchRingTonePicker(Alarm alarm) {
...
private
void saveRingtoneUri(Intent intent) {
...
class AlarmItemAdapter extends CursorAdapter {
	...
	public
	AlarmItemAdapter(Context context,
	                 long[] expandedIds,
	                 long[] repeatCheckedIds,
	                 long[] selectedAlarms,
						...
						if(firstAnimation) {
							...
							child.animate().withEndAction(new Runnable() {
								...
								void run() {
									...
									if(addedView != null) {
										addedView.animate().alpha(1.0f).setDuration(ANIMATION_DURATION).withEndAction(
												new Runnable() {
	...
	@Override
	public
	void bindView(final View view, Context context, final Cursor cursor) {
		...
		itemHolder.clock.setFormat((int) mContext.getResources()
		                                         .getDimension(R.dimen.alarm_label_size));
		...
		final String daysOfWeekStr = alarm.daysOfWeek.toString(AlarmClockFragment.this.getActivity(),
		                                                       false);
	...
	private
	void bindExpandArea(final ItemHolder itemHolder, final Alarm alarm) {
		...
		itemHolder.ringtone.setContentDescription(mContext.getResources()
		                                                  .getString(R.string.ringtone_description) +
		                                          " " +
		                                          ringtone);
	...
	private
	void setItemAlpha(ItemHolder holder, boolean enabled) {
	...
	private
	void updateDaysOfWeekButtons(ItemHolder holder, DaysOfWeek daysOfWeek) {
	...
	private
	void turnOffDayOfWeek(ItemHolder holder, int dayIndex) {
	...
	private
	void turnOnDayOfWeek(ItemHolder holder, int dayIndex) {
	 ...
	 */
	private
	void expandAlarm(final ItemHolder itemHolder, boolean animate) {
		...
		observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
			...
			boolean onPreDraw() {
				...
				FrameLayout.LayoutParams
						expandParams =
						(FrameLayout.LayoutParams) itemHolder.expandArea.getLayoutParams();
	...
	private
	boolean isAlarmExpanded(Alarm alarm) {
		return mExpanded.contains(alarm.id);
	}

	private
	void collapseAlarm(final ItemHolder itemHolder, boolean animate) {
	...
	private
	void buildHashSetFromArray(long[] ids, HashSet<Long> set) {
		for (long id : ids) {
			set.add(id);
		}
	}
...
private static
AlarmInstance setupAlarmInstance(Context context, Alarm alarm) {
...
private
void asyncDeleteAlarm(final Alarm alarm, final View viewToRemove) {
	...
	final AsyncTask<Void, Void, Void> deleteTask = new AsyncTask<Void, Void, Void>() {
		...
		@Override
		protected
		Void doInBackground(Void... parameters) {
...
void asyncAddAlarm(final Alarm alarm) {
	final Context context = AlarmClockFragment.this.getActivity().getApplicationContext();
	final
	AsyncTask<Void, Void, AlarmInstance>
			updateTask =
			new AsyncTask<Void, Void, AlarmInstance>() {
				...
				@Override
				protected
				AlarmInstance doInBackground(Void... parameters) {
					if(context != null && alarm != null) {
						ContentResolver cr = context.getContentResolver();
				...
				@Override
				protected
				void onPostExecute(AlarmInstance instance) {
					if(instance != null) {
